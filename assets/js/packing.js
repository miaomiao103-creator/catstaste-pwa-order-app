...
function buildShipmentWhatsappText(order, courierCompany, trackingNo) {
  return `Hi ${order.customerName || ''}，你嘅 CatsTaste 訂單已安排寄出。\n\n訂單編號：${order.orderId || ''}\n快遞公司：${courierCompany || ''}\n快遞單號：${trackingNo || ''}\n\n你可以用以上單號查詢物流狀態。\n如有問題歡迎 WhatsApp 我哋，多謝支持。`;
}
...
function currentVerifyOrder() {
  return ((latestSheetDashboard && latestSheetDashboard.todayOrderCards) || []).find(o => String(o.orderId) === String(verifyOrderId));
}

function updateShipmentPreview() {
  const order = currentVerifyOrder();
  if (!order) return;
  const courierCompany = $('verifyCourierCompany').value.trim();
  const trackingNo = $('verifyTrackingNo').value.trim();
  $('shipNoticePreview').value = buildShipmentWhatsappText(order, courierCompany, trackingNo);
}

async function markOrderShippedOnServer(order, courierCompany, trackingNo) {
  const shippedAt = nowHKStamp();
  const payload = {
    ...
    trackingNo,
    shippedAt,
    shipmentNoticeText: buildShipmentWhatsappText(order, courierCompany, trackingNo)
  };
  ...
  if (!latest || String(latest.fulfillmentStatus || 'pending') !== 'shipped') {
    throw new Error('已寄出狀態未成功寫入 Google Sheet');
  }
...
async function scanLoop() {
  const video = $('scanVideo');
  if (!video || !barcodeDetector || !scanStream) return;
  try {
    const codes = await barcodeDetector.detect(video);
    if (codes && codes.length) {
      const rawValue = String(codes[0].rawValue || '').trim();
      if (rawValue) {
        $('verifyTrackingNo').value = rawValue;
        updateShipmentPreview();
        $('statusLine').textContent = `已掃描到快遞單號：${rawValue}`;
        stopTrackingScanner();
        return;
      }
    }
  } catch (err) {
    console.warn('scan detect failed', err);
  }
...
async function startTrackingScanner() {
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    $('statusLine').textContent = '呢部裝置未支援鏡頭掃描，請手動輸入快遞單號。';
    return;
  }
  if (!('BarcodeDetector' in window)) {
    $('statusLine').textContent = '此瀏覽器未支援 barcode / QR code 掃描，請手動輸入快遞單號。';
    return;
  }
  try {
    const formats = ['qr_code','code_128','code_39','code_93','ean_13','ean_8','upc_a','upc_e','itf'];
    let supported = formats;
    if (BarcodeDetector.getSupportedFormats) {
      try { supported = await BarcodeDetector.getSupportedFormats(); } catch {}
    }
    barcodeDetector = new BarcodeDetector({ formats: formats.filter(f => supported.includes(f)) });
    ...
    await video.play();
    $('statusLine').textContent = '掃描中，請將快遞單號 barcode / QR code 對準鏡頭。';
    scanLoop();
  } catch (err) {
    stopTrackingScanner();
    $('statusLine').textContent = '未能開啟鏡頭掃描，請檢查權限或改用手動輸入。';
  }
}
...
function closeVerifyModal() {
  ...
  verifyOrderId = '';
  $('verifyCourierCompany').value = '';
  $('verifyTrackingNo').value = '';
  $('shipNoticePreview').value = '';
}
...
function openVerifyModal(order) {
  ...
  $('verifyCourierCompany').value = order.courierCompany || 'SF Express';
  $('verifyTrackingNo').value = order.trackingNo || '';
...
async function handleOrderAction(action, orderId) {
  ...
  if (action === 'copyShipNotice') return copyText($('shipNoticePreview').value, `已複製 ${orderId} 嘅出貨通知。`);
  ...
  if (action === 'confirmShipped') {
    ...
    const trackingNo = $('verifyTrackingNo').value.trim();
    if (!courierCompany) {
      $('statusLine').textContent = '請先選擇快遞公司。';
      return;
    }
    if (!trackingNo) {
      $('statusLine').textContent = '請先輸入或掃描快遞單號。';
      return;
    }
    try {
      $('statusLine').textContent = `正在更新 ${orderId} 為已寄出...`;
      await markOrderShippedOnServer(order, courierCompany, trackingNo);
...
function renderOrdersBoard(filteredOrders) {
  $('ordersBoard').innerHTML = filteredOrders.map(order => {
    ...
    const shipmentInfo = [order.courierCompany ? `快遞：${htmlEscape(order.courierCompany)}` : '', order.trackingNo ? `單號：${htmlEscape(order.trackingNo)}` : ''].filter(Boolean).join(' · ');
    return `<div class="order-card ${cls}">
      <div class="order-card-top">
        <div>
          ...
          <div class="order-card-meta">${htmlEscape(order.createdAt || '')} · ${htmlEscape(order.staffName || '')} · ${htmlEscape(order.deviceId || '')} · 同步 ${htmlEscape(syncStatusLabel(order.syncStatus))}${shipmentInfo ? ` · ${shipmentInfo}` : ''}</div>
        </div>
...
function renderPackingPage() {
  ...
  if (!data || !data.ok) {
    ...
    $('statusLine').textContent = '未下載資料。請有網時按「更新待執貨清單」，之後冇網都可以開呢頁用最後一次快取。';
  ...
  const orders = ((data && data.todayOrderCards) || []).slice().sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
  const filtered = rows.filter(row => matchesPackingFilter(row));
  ...
  $('statusLine').textContent = currentView === 'packing'
    ...
    : `${packingDashboardAgeText(data)} · ${online ? '可更新最新資料' : '目前離線，正使用快取'} · 按時間列出 ${orders.length}/${Number(data.todayOrders || 0)} 張訂單`;
  syncFilterButtons();

  $('packingList').innerHTML = filtered.map(row => {
    const theme = productSeriesTheme(row.category);
    const orderRows = row.orders.slice().sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || ''))).map(order => `
      <div class="order-row">
        <div>
...
async function refreshSheetDashboard() {
  ...
  try {
    $('statusLine').textContent = '正在讀取 Google Sheet 待執貨資料...';
...
$('refreshBtn').addEventListener('click', () => refreshSheetDashboard());
...
$('verifyConfirmBtn').addEventListener('click', () => {
  ...
  handleOrderAction('confirmShipped', verifyOrderId);
});
$('verifyCourierCompany').addEventListener('change', updateShipmentPreview);
$('verifyTrackingNo').addEventListener('input', updateShipmentPreview);
$('copyShipNoticeBtn').addEventListener('click', () => {
  if (!verifyOrderId) return;
  handleOrderAction('copyShipNotice', verifyOrderId);
});
$('startScanBtn').addEventListener('click', startTrackingScanner);
$('stopScanBtn').addEventListener('click', stopTrackingScanner);
...
(async function boot() {
  ...
  loadPackingDashboardCache();
  renderPackingPage();
  if (navigator.onLine) refreshSheetDashboard();
})();
