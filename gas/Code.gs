...
const HEADERS = [
  ...
  'paymentMethod','paymentStatus','shippingMethod','shippingFreeEligible','shippingArrangement','retailSubtotal','subtotal','boxDiscountAmount','discountAmount',
  'discountedSubtotal','totalDiscountAmount','total','giftEligible',
  'fulfillmentStatus','syncStatus','syncedAt','lastSyncError','itemsJson','whatsappText','courierCompany','trackingNo','shippedAt','shipmentNoticeText','serverReceivedAt','staffNotes'
];
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'orderStatus') {
      const ss = SpreadsheetApp.getActiveSpreadsheet();
      return output_(params, getOrderStatus_(ss, params));
    }
...
function getOrderStatus_(ss, params) {
  const orderId = String(params && params.orderId || '').trim();
  if (!orderId) throw new Error('Missing orderId');
  const sheet = getOrCreateSheet_(ss);
  const existingRow = findOrderRow_(sheet, orderId);
  if (!existingRow) return { ok: true, found: false, orderId };

  const current = readOrders_(ss).find(o => String(o.orderId) === orderId);
  if (!current) return { ok: true, found: false, orderId };

  const shouldMarkVerified = String(params && params.markVerified || '') === '1';
  if (shouldMarkVerified && String(current.syncStatus || '') !== 'verified' && String(current.syncStatus || '') !== 'synced') {
    current.syncStatus = 'verified';
    current.syncedAt = new Date().toISOString();
    current.lastSyncError = '';
    const row = HEADERS.map(h => {
      if (h === 'itemsJson') return JSON.stringify(current.items || []);
      if (h === 'serverReceivedAt') return current.serverReceivedAt || new Date().toISOString();
      return current[h] === undefined || current[h] === null ? '' : current[h];
    });
    sheet.getRange(existingRow, 1, 1, HEADERS.length).setValues([row]);
    rebuildHelperSheets_(ss);
  }

  const latest = readOrders_(ss).find(o => String(o.orderId) === orderId) || current;
  return {
    ok: true,
    found: true,
    orderId,
    ...
    courierCompany: latest.courierCompany || '',
...
function shippingArrangementText_(order) {
  const arrangement = String(order && order.shippingArrangement || '').toLowerCase();
  const freeFlag = order && (order.shippingFreeEligible === true || String(order.shippingFreeEligible).toLowerCase() === 'true');
  const total = Number(order && order.total || 0);
  return arrangement === 'free' || freeFlag || total >= 800 ? '滿 HK$800：免費送貨' : '未滿 HK$800：會安排到付';
}
...
function writeOrdersView_(ss, orders) {
  const headers = ['出貨狀態','Order No','時間','同事','設備','客人','WhatsApp','地址','應收','送貨安排','產品','備註'];
  const rows = orders.slice().sort((a,b) => String(b.createdAt||'').localeCompare(String(a.createdAt||''))).map(o => [
    ...
    o.customerName || '', o.phone || '', o.address || '', o.total || 0, shippingArrangementText_(o), orderItemsText_(o), o.notes || o.staffNotes || ''
  ]);
...
function compactOrderForApp_(o) {
  return {
    ...
    shipmentNoticeText: o.shipmentNoticeText || '',
    items: (o.items || []).map(i => ({
      sku: i.sku || '',
      name: i.name || '',
      qty: Number(i.qty || 0),
